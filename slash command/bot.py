...
@bot.slash_command(description="anti link command")
@nextcord.ext.application_checks.has_permissions(administrator=True)
async def anti_insult(interaction: Interaction): 
    try:
        with open("anti-insult-system.json", "r") as f: # Opening JSON file in variable "f"
            antilink = json.load(f) # Parsing it to a Python dictionary.
        anti_link = antilink[str(interaction.guild.id)] # Getting the Guild's Value via it's value.

    except KeyError: # If guild toggle isnt in JSON file, Adding it.
        antilink[str(interaction.guild.id)] = "on"
    ...
    if anti_link == 'off':
        ...
        antilink[str(interaction.guild.id)] = anti_link # setting the guild toggle to 'off'
